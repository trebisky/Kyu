...
/* This seemed to be working fine for armv7, but we have not yet
 * implemented the MMU support routines for armv8, so we turn this
 * off, which will work for v7 and for v8.
#define EMAC_NOCACHE
 */

/* In armv8/cache_v8.c */
void	flush_dcache_range ( addr_t, addr_t );
void	invalidate_dcache_range ( addr_t, addr_t );
...
rx_list_show ( struct emac_desc *desc, int num )
{
	...
	emac_cache_invalidate ( (addr_t) desc, (addr_t) &desc[num] );
 ...
 */
	...
	emac_cache_flush ( (addr_t) list, (addr_t) &list[num] );
}
...
kyu_receive ( char *buf, int len )
{
	...
	// invalidate_dcache_range ( (u32) buf, (u32) &buf[UBOOT_PKTSIZE] );
	emac_cache_invalidate ( (addr_t) buf, (addr_t) &buf[UBOOT_PKTSIZE] );
...
rx_handler ( int stat )
{
	...
	while ( ! (cur_rx_dma->status & DS_ACTIVE) ) {
	    ...
	    emac_cache_flush ( (addr_t) cur_rx_dma, (addr_t) &cur_rx_dma[1] );
