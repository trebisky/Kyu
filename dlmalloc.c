/* This is the famous David Lea malloc package
 * Taken from U-boot sources (2014.01 common/dlmalloc.c)
 * Tweaked into shape for Kyu  5-26-2015
 *
 * Note that this is version 2.6.6 circa 2000
 * as of 5-2015 version 2.8.6 is available
 *  (with almost twice as many lines of code).
 */
...
#ifdef KYU
/* This is code added by U-boot that seems perfectly
 * good for Kyu.  The game is to provide an sbrk() function
 * which is the only hook dlmalloc needs to get raw memory.
 * We must initialize this subsystem by a call to
 *  mem_malloc_init() giving the start address and size
 *  of the block of memory to be used by dlmalloc
 */
...
void
mem_malloc_init ( ulong start, ulong size )
{
	ulong val;
