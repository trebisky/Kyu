...
/* Copied from U-boot 2015.01 sources
 ...
 */
int
genphy_restart_aneg(struct phy_device *phydev)
{
	int ctl;
 ...
 */
int
genphy_config_aneg(struct phy_device *phydev)
{
	int result;
 ...
 */
int
genphy_update_link(struct phy_device *phydev)
{
 ...
 */
int
genphy_parse_link(struct phy_device *phydev)
{
...
int
genphy_config(struct phy_device *phydev)
{
	int val;
	u32 features;
...
int
genphy_startup(struct phy_device *phydev)
{
...
int
genphy_shutdown(struct phy_device *phydev)
{
	return 0;
}
...
static int
phy_probe(struct phy_device *phydev)
{
	int err = 0;
 ...
 */
static int
get_phy_id(struct mii_dev *bus, int addr, int devad, u32 *phy_id)
{
	int phy_reg;
...
static struct phy_device *
search_for_existing_phy(struct mii_dev *bus,
 ...
 */
static struct phy_device *
get_phy_device(struct mii_dev *bus, int addr,
					 phy_interface_t interface)
{
...
int
phy_reset(struct phy_device *phydev)
{
	int reg;
	int timeout = 500;
...
void
phy_connect_dev(struct phy_device *phydev, struct eth_device *dev)
{
	/* Soft Reset the PHY */
	phy_reset(phydev);
	if (phydev->dev) {
...
struct phy_device *
phy_connect(struct mii_dev *bus, int addr,
 ...
 */
int
phy_startup(struct phy_device *phydev)
{
...
int 
phy_config(struct phy_device *phydev)
{
...
int
phy_shutdown(struct phy_device *phydev)
{
