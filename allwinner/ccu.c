 ...
 * more details.
 *
 * ccu.c for the Allwinner H3 and H5
 *           (the Orange Pi PC and PC 2)
 *
 ...
 * It is labeled R_PRCM in the documents, but essentially
...
#define R_CCU_BASE	(struct r_ccu *) 0x01f01400
...
/* I am too lazy to calculate the pad values to
 * stick this register into the above structure.
...
spin_clocks_on ( void )
{
	...
	lp = (vu32 *) (CCU_BASE_VAL + BUS_GATING_REG1);
	*lp |= SPINLOCK_BIT;
	lp = (vu32 *) (CCU_BASE_VAL + BUS_RESET_REG1);
	*lp |= SPINLOCK_BIT;
}
